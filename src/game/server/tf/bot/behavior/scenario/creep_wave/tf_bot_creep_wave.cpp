...
ConVar tf_creep_usecustommodel("tf_creep_usecustommodel", "0");
ConVar tf_creep_model("tf_creep_model", "models/bots/heavy/bot_heavy.mdl");
...
ActionResult< CTFBot >	CTFBotCreepWave::OnStart( CTFBot *me, Action< CTFBot > *priorAction )
{
	...
	if (tf_creep_usecustommodel.GetBool() == true)
	{
		me->GetPlayerClass()->SetCustomModel(tf_creep_model.GetString(), USE_CLASS_ANIMATIONS);
	}
	me->UpdateModel();
