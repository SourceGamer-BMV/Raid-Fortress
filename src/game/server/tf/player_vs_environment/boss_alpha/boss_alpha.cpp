...
ConVar tf_boss_alpha_takedamage_fix("tf_boss_alpha_takedamage_fix", "0"/*, FCVAR_CHEAT */);

ConVar tf_boss_alpha_scale("tf_boss_alpha_scale", "1"/*, FCVAR_CHEAT */);

ConVar tf_boss_alpha_adv_ignore("tf_boss_alpha_adv_ignore", "0"/*, FCVAR_CHEAT */);
...
BEGIN_DATADESC( CBossAlpha )
    DEFINE_KEYFIELD(m_nForceModel, FIELD_INTEGER, "forcemodel"),
    DEFINE_KEYFIELD(m_iszCustomModel, FIELD_STRING, "custommodel"),
...
void CBossAlpha::Precache()
{
	...
	int model = PrecacheModel( "models/bots/engineer/bot_engineer.mdl" );
#else
	int model = PrecacheModel( "models/player/heavy.mdl" );
#endif

	if (m_nForceModel == 1)
	{
		int model = PrecacheModel(m_iszCustomModel);
	}
...
#ifdef USE_BOSS_SENTRY
	SetModel( "models/bots/engineer/bot_engineer.mdl" );
#else
	SetModel( "models/player/heavy.mdl" );
#endif

	SetModelScale(tf_boss_alpha_scale.GetFloat());
	...
	case HITBOX_SKILL_PRECISION_3:
		{
			...
			if ( !m_isPrecisionShotDone && !m_isPrecisionShotHit[ which ] )
			{
				...
				UTIL_ClientPrintAll( HUD_PRINTCENTER, CFmtStr( "PRECISION SHOT %d...", which+1 ) );
				...
				if ( i == PRECISION_SHOT_COUNT )
				{
					...
					UTIL_ClientPrintAll( HUD_PRINTCENTER, "PRECISION SKILL SHOT!" );
	...
	case HITBOX_SKILL_DAMAGE_SPONGE:
		if ( !m_isDamageSpongeSkillShotDone )
		{
			...
			if ( m_damageSpongeSkillShotAmount > tf_boss_alpha_skill_shot_damage_sponge_total.GetFloat() )
			{
				...
				m_damageSpongeSkillShotAmount = 0.0f;
				UTIL_ClientPrintAll( HUD_PRINTCENTER, "DAMAGE SPONGE SKILL SHOT!" );
				EmitSound( "RobotBoss.DamageSpongeSkillShot" );
				OnSkillShot();
				return true;
			}

			UTIL_ClientPrintAll(HUD_PRINTCONSOLE, CFmtStr( "DAMAGE SPONGE = %3.2f", m_damageSpongeSkillShotAmount ) );
			return true;
		}
		break;
			...
			if ( info.GetDamage() > tf_boss_alpha_skill_shot_hard_hit_damage.GetFloat() )
			{
				...
				if ( info.GetAttacker() )
				{
					...
					if ( toAttacker.z > tf_boss_alpha_skill_shot_hard_hit_z.GetFloat() )
					{
						...
						UTIL_ClientPrintAll( HUD_PRINTCENTER, "HARD HIT SKILL SHOT!" );
...
void CBossAlpha::UpdateSkillShots( void )
{
	...
	{
		UTIL_ClientPrintAll( HUD_PRINTCONSOLE, CFmtStr( "DAMAGE SPONGE = %3.2f/%3.2f", m_damageSpongeSkillShotAmount, tf_boss_alpha_skill_shot_damage_sponge_total.GetFloat() ) );
	}
	...
	if ( m_skillShotComboTimer.HasStarted() && m_skillShotComboTimer.IsElapsed() )
	{
		...
		ResetSkillShots();
		UTIL_ClientPrintAll( HUD_PRINTCONSOLE, "SKILL SHOT CHAIN FAILED - TOO SLOW!" );
	}
	...
	if ( !m_isPrecisionShotDone && m_precisionSkillShotTimer.HasStarted() && m_precisionSkillShotTimer.IsElapsed() )
	{
		...
		UTIL_ClientPrintAll( HUD_PRINTCONSOLE, "PRECISION SHOTS RESET - TOO SLOW!" );
	}
}
...
int CBossAlpha::OnTakeDamage_Alive( const CTakeDamageInfo &rawInfo )
{
	...
	if (tf_boss_alpha_takedamage_fix.GetBool() == false)
	{
	...
	if ( info.GetAttacker() && info.GetAttacker()->MyCombatCharacterPointer() && !InSameTeam( info.GetAttacker() ) )
	{
		...
		if (tf_boss_alpha_takedamage_fix.GetInt() == 2)
		{
			float prehealth = GetHealth();
			float posthealth = prehealth - RandomFloat(3, 30);
			SetHealth(posthealth);
		}
		if (tf_boss_alpha_takedamage_fix.GetInt() == 3)
		{
			float prehealth = GetHealth();
			float posthealth = prehealth - info.GetDamage();
	        
			if (info.GetDamageType() & DMG_CRITICAL)
			{
				posthealth = prehealth - info.GetDamage() * -TF_DAMAGE_CRIT_MULTIPLIER;
			}
...
bool CBossAlpha::IsIgnored( CTFPlayer *player ) const
{
	if ( player->m_Shared.IsStealthed() )
	{
		...
		{
			// always notice with these conditions
			return false;
		}
	...
	if (tf_boss_alpha_adv_ignore.GetBool() == true)
	{
		if (player->m_Shared.InCond(TF_COND_INVULNERABLE) || player->m_Shared.InCond(TF_COND_INVULNERABLE_USER_BUFF) || player->m_Shared.InCond(TF_COND_INVULNERABLE_CARD_EFFECT))
		{
			return true;
		}
	}
